...
with DAG(
    dag_id="dataflow_backfill_dag",
    default_args=default_args,
    description="Backfill historical data using Dataflow",
    schedule_interval=None,
    start_date=days_ago(1),
    catchup=False,
    tags=["dataflow", "batch", "backfill"],
) as dag:

    backfill_job = DataflowStartFlexTemplateOperator(
        task_id="run_backfill_job",
        body={
            "launchParameter": {
                "jobName": "backfill-job-{{ ds_nodash }}",
                ...
                "parameters": {
                    ...
                    # Optional: extend your pipeline to support date filtering
                    "start_date": "{{ ds }}",
                    "end_date": "{{ next_ds }}"
                },
                "environment": {
